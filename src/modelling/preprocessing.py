...
@task(name="Encode categorical columns", tags=["Encode categorical columns"], retries=2, retry_delay_seconds=60)
def encode_categorical_cols(df: pd.DataFrame) -> pd.DataFrame:
    """Encode categorical columns as dummy variables."""
    return pd.get_dummies(df)


@task(name="Extract X and y", tags=["Extracting X and y"], retries=2, retry_delay_seconds=60)
